...
import { fetchActiveQueues, queueMeIn } from '../api';
...
export const ConfirmTokenScreen = () => {
  ...
  const loadQueues = async () => {
    ...
    try {
      ...
      console.log('⚠️ Debugging: Skipping getCurrentLocation');
      // const location = await getCurrentLocation();
      // lat = location.latitude;
      // long = location.longitude;
      // console.log('📍 Current Location:', lat, long);
      throw new Error('Skipping location for debug');
    } catch (error) {
  ...
  const handleBookToken = async () => {
    ...
    if (result.success) {
      Alert.alert('Success', result.message, [
        { text: 'OK', onPress: () => navigation.navigate('My Tokens') }, // Navigate to My Tokens on success
      ]);
    } else {
